    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex flex-col items-center">
              <div className="relative">
                <div className="w-24 h-24 rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 p-1 shadow-lg">
                  <div className="w-full h-full rounded-full bg-slate-900 text-white flex items-center justify-center text-4xl">
            ...
            <div className="flex-1">
              {isEditingProfile ? (
                <div className="space-y-4">
                        ...
                        <button
                          ...
                          className={`w-10 h-10 rounded-lg border-2 text-xl transition-all ${
                            tempAvatar === avatar 
                              ...
                              : 'border-slate-600 hover:border-slate-400 text-white bg-slate-900'
                          }`}
                        >
                          {avatar}
                        </button>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="btn bg-green-600 text-white hover:bg-green-500 flex items-center gap-1"
                    >
                      <Save size={16} />
                      Save
                    </button>
                ...
                <div>
                  <h1 className="text-2xl font-bold text-white mb-1">{state.settings.username}</h1>
                  <p className="text-lg text-blue-300 font-medium mb-2 flex items-center gap-2">
                    <Crown size={20} />
            ...
            <div className="text-right">
              <div className="flex items-center gap-2 bg-gradient-to-r from-yellow-400 to-orange-500 text-black px-4 py-2 rounded-full shadow-lg">
                <span className="text-xl">🪙</span>
                <span className="font-bold text-lg">{state.coins}</span>
              </div>
              <p className="text-xs text-slate-400 mt-1">MotiveCoins</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-white">Level {state.userStats.level.level}</div>
              <div className="text-sm text-slate-300">{state.userStats.level.title}</div>
            </div>
            <div className="text-right">
              <div className="text-lg font-semibold text-blue-300">{state.userStats.totalXP.toLocaleString()} XP</div>
              <div className="text-sm text-slate-300">{state.userStats.level.xpToNext} to next level</div>
            </div>
          </div>

          <div className="w-full bg-slate-800 rounded-full h-4 overflow-hidden ring-1 ring-white/10">
            <div
      ...
      <div className="card">
        ...
        {recentAchievements.length === 0 ? (
          <div className="text-center py-8 text-slate-400">
            <Trophy size={48} className="mx-auto text-slate-500 mb-2" />
          ...
          <div className="space-y-3">
            {recentAchievements.map((achievement) => (
              <div key={achievement.id} className="flex items-center gap-3 p-3 bg-slate-800/60 rounded-lg border border-white/10">
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm text-blue-300 font-medium">+{achievement.points} XP</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="card">
        <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
          ...
          <span className="text-white">Theme Settings</span>
        </h2>
        ...
        <div className="grid grid-cols-1 gap-3">
          {Object.values(themes).map((theme) => (
            ...
            >
              ...
              {state.settings.theme === theme.id && (
                <Star size={16} className="text-blue-400 ml-auto" />
              )}
            </button>
          ))}
        </div>
        
        <div className="mt-4 p-3 bg-slate-800 rounded-lg border border-white/10">
          <p className="text-sm text-slate-300">
            <span className="font-medium text-white">Current theme:</span> {themes[state.settings.theme]?.name || 'Default'}
          </p>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-300">Get notified about streaks and achievements</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              ...
              />
              <div className="w-11 h-6 bg-slate-700 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300/30 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-500 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-300">Play sounds for achievements and completions</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
